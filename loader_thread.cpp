...
logparser::parser::parser() {
  regexers.insert(
      {"switchname", regex{"(?:MSCi +)(\\w+)(?: +)(\\d{4}-\\d{2}-\\d{2} "
                           "+\\d{2}\\:\\d{2}\\:\\d{2})"}});
  regexers.insert(
      {"cell_2g", regex{"(?:^ +)(\\w+)(?: +\\d+ +)(\\d+)(?: +)(\\d+)(?: "
                        "+)(\\d+)(?: +)(\\d+)(?: +)(\\w+)"}});
  regexers.insert(
      {"rncname", regex{"(?:RNC NAME\\.+ RNCNAME \\. \\: )(\\w+)"}});
  ...
  regexers.insert(
      {"enodeb_id",
       regex{"(?:ENB IDENTIFICATION\\.+\\(ENBID\\)\\.+ \\: )(\\d+)"}});
  regexers.insert({"inodeb_ip", regex{"(?:ENB IP ADDRESS\\.+\\(ENBIP\\)\\.+ "
                                      "\\: )(\\d+\\.\\d+\\.\\d+\\.\\d+)"}});
  regexers.insert(
      {"tac", regex{"(?:TRACKING AREA CODE\\.+\\(TAC#1 \\)\\.+ \\: )(\\d+)"}});
  triggers.insert({"cell_3g", regex{"SERVICE AREAS IN MSS CONCEPT \\:"}});
  triggers.insert({"cell_2g", regex{"BTSs UNDER BSC \\:"}});
  triggers.insert(
      {"bscname", regex{"(?:BSC NAME )(?:\\.*)(?:\\(NAME\\)\\. \\:)(\\w+)"}});
  triggers.insert(
      {"rncid", regex{"(?:RNC IDENTIFICATION\\.+ RNCID \\.+ \\: )(\\d+)"}});
  triggers.insert({"rnc_load_lac",
                   regex{"(?:LA +NAME \\:LAC)(\\d+)(?: +LAC +\\: *)(\\d+)"}});
}
logparser::parser::~parser() {}
void logparser::parser::slot_new_file(
    const string &filename,
    const logtype &type_of_logger) throw(const domain_error &) {
...
void logparser::parser::slot_run() throw(const runtime_error &) {
  ...
  signal_n_of_mobswitches_ready_.emit(to_string(working_switches.size()));
  boost::this_thread::sleep_for(boost::chrono::milliseconds(400));
}

void logparser::parse_switch(std::shared_ptr<::mobswitch> work_switch,
                             const string &filename,
                             const map<string, regex> &triggers,
                             const map<string, regex> &regexers) {
  // DEBUG
  ...
  while (workfile.good()) {
    ...
    if (regex_search(linha, matches, triggers.at("cell_2g"))) {
      if (!t_bsc)
        throw(runtime_error("Começaram celulas 2g sem descobrir uma bsc. "
                            "Travando em 3, 2, 1..."));
    ...
    if (regex_search(linha, matches, triggers.at("cell_3g"))) {
      if (work_lac_sac < 0)
        throw(runtime_error("Começaram celulas 3g sem que fosse possível "
                            "descobrir o SAC. Travando em 3, 2, 1..."));
      t_cell2g = t_bsc = t_rnc = false;
      t_cell3g = true;
      continue;
    }
...
void logparser::parse_mme(std::shared_ptr<::mme>, const string &filename,
